...
5. **Mill migration** — delete `PoolAutocomplete` + `RaydiumPoolAutocomplete` and the `if (lane === "raydium")` branch in `param-field.tsx`; embed `pools-client`'s `PoolRow`/`PoolPicker`, wire `usePoolSearch(q, {baseUrl}, {venue: lane})`, thread `extras.ammConfig` into `onPoolSelect` (replaces the positional 6th arg). Preserves non-custodial + one-signature invariants (setup-time param source only). **Lives in the `mill/` repo (separate).**
...
| Mill consumes the unified picker (clones deleted)                                                                   | ✗ pending §6.5 (lives in `mill/` repo)                       |
