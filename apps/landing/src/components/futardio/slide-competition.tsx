  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full px-3 sm:px-8">
      ...
      >
        ...
        protocol at a time. First-mover is technical lock-in.
      </motion.p>

      <motion.div
        className="max-w-5xl w-full overflow-x-auto mb-6"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {wallets.map((w) => (
            <div key={w.name} className="border border-border bg-muted/30 p-3">
              ...
              <div className="text-sm text-muted-foreground mb-1">{w.what}</div>
              <div className="text-sm text-emerald-400">{w.play}</div>
            </div>
          ))}
        </div>
      </motion.div>
