...
export default class ReferralChain extends BaseCommand {
  ...
  static flags = {
    ...
    owner: Flags.string({char: 'o', description: 'Owner public key to trace chain for', required: true}),
  }
  ...
  public async run(): Promise<void> {
    ...
    this.output({
      chain: {L1: chain[0]?.toString() ?? null, L2: chain[1]?.toString() ?? null, L3: chain[2]?.toString() ?? null},
