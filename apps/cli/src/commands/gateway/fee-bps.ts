...
export default class GatewayFeeBps extends BaseCommand {
  ...
  static flags = {
    ...
    'fee-bps': Flags.string({char: 'b', description: 'New fee in basis points', required: true}),
  }
  ...
  public async run(): Promise<void> {
    ...
    const signature = await this.send(await sdk.changeGatewayFeeBps(authority, feeBps))
