...
export default class GatewayFeeRecipient extends BaseCommand {
  ...
  static flags = {
    ...
    'new-recipient': Flags.string({char: 'r', description: 'New fee recipient public key', required: true}),
  }
  ...
  public async run(): Promise<void> {
    ...
    if (!newRecipient) this.error('Invalid new recipient public key')

    const sdk = await this.getSDK()
    const signature = await this.send(await sdk.changeGatewayFeeRecipient(authority, newRecipient))
