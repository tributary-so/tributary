...
export default class GatewaySigner extends BaseCommand {
  ...
  static flags = {
    ...
    authority: Flags.string({char: 'a', description: 'Current gateway authority public key', required: true}),
    'new-signer': Flags.string({char: 's', description: 'New signer public key', required: true}),
  }
  ...
  public async run(): Promise<void> {
    ...
    if (!newSigner) this.error('Invalid new signer public key')

    const sdk = await this.getSDK()
    const signature = await this.send(await sdk.changeGatewaySigner(authority, newSigner))
