...
| **Execution**              | Single CPI: `transfer` from user → recipient + fees.                                  | 3-phase: `PULL` → `VALIDATE` (opt) → `FORWARD` (opt) → `SETTLE`.                                          |
...
| **Fee math**               | `shared::fees::calculate_fees`                                                        | Same — `shared::fees::calculate_fees`. Single source of truth (M7).                                       |
