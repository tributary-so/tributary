...
- **apps/api/src/**tests**/token-issuer.test.ts** — 14 new unit tests covering all 5 variants (claim shape + status transitions + exp derivation). All pass.
- **apps/docs/adr/0025-jwt-payload-generalized-policyclaim-union.md** — new ADR documenting the decision, rejected alternatives, and rationale. (Renumbered from 0024 → 0025 to resolve a collision with the PayAsYouGo-expiration ADR that landed earlier the same day.)
- **AGENTS.md** — ADR-0025 wired into the ADR map.
