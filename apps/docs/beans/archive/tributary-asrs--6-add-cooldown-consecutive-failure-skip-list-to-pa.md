...
- **`apps/scheduler/src/payments.ts`**: Added `CooldownEntry` interface, `cooldowns` map, and `recordFailure()` method mirroring `ComposableScheduler`'s pattern. Cooldown check runs before `shouldExecutePayment` — policies on cooldown are skipped with a debug log. On success, the cooldown entry is deleted (re-eligible next cycle). On failure, `recordFailure()` increments the strike count; after `MAX_FAILURES=3`, the policy enters a `COOLDOWN_MS=5min` cooldown. Cooldowns persist across cron ticks (not cleared at cycle start).
