...
const models = [
  ...
  },
  {
    ...
    example: 'Freelance, consulting, dev bounties — escrow without the middleman.',
    icon: '4p',
      ...
      <div className="grid grid-cols-3 gap-3 max-w-3xl w-full mb-2">
        {models.slice(0, 3).map((model, i) => (
          ...
          >
            ...
            <div className={`text-[10px] uppercase tracking-[0.2em] font-bold ${model.text} mb-1`}>{model.name}</div>
            <div className="text-xs text-foreground mb-1">{model.desc}</div>
            <div className="text-[10px] text-muted-foreground leading-snug">{model.example}</div>
          </motion.div>
        ))}
      </div>
