...
export class PaymentPolicyTracker {
   ...
   * The SDK then translates each present field into the correct PaymentPolicy
   * memcmp offset (user_payment=8, recipient=40, gateway=72, memo=234).
   */
  private buildPaymentFilters(options: PolicyLookupOptions): {
  ...
  /**
   * Fetch every PaymentPolicy matching the supplied lookup options.
   * Delegates to {@link Tributary.getPaymentPolicies} (SDK owns the memcmp
   * offsets) and returns the raw accounts unchanged.
   */
  async getPaymentPoliciesForOptions(
    options: PolicyLookupOptions
  ): Promise<Array<{ publicKey: PublicKey; account: PaymentPolicy }>> {
    const filters = this.buildPaymentFilters(options);
    return await this._tributary.getPaymentPolicies(filters);
  }
}
...
/**
 * ComposablePolicyTracker — mirrors {@link PaymentPolicyTracker} for the
...
export class ComposablePolicyTracker {
   ...
   *
   * `walletPublicKey` + `tokenMint` are paired into a derived user-payment PDA
   * (mirrors {@link PaymentPolicyTracker.getPaymentPoliciesForOptions}). The SDK
