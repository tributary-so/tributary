...
describe("Composable Topup-Swap Flow (USDC → WSOL via Meteora DLMM)", () => {
  ...
  // Output side (WSOL) — only the recipient ATA is output-mint (the forward
  // delivers WSOL to the recipient via the deliver-transform sweep).
  ...
  beforeAll(async () => {
    ...
    // Create ATAs (input USDC for coldWallet + fee accounts; output WSOL for
    // recipient). Fee accounts are input-side post-ADR-0026.
  ...
  test("Create composable swap policy — DLMM forward USDC→WSOL + Lighthouse", async () => {
    ...
    // Forward ENABLED: Meteora DLMM swap (USDC → WSOL). The
    // instructionConstraint pins the swap selector (byte-range check at
    // offset 0) AND pins the first forward account (degenerate-pin guard +
    // execute-time positional pin-check, ADR-0021). swapIx.keys[0] is the
    // lbPair — the first account remaining_mid[fwd_base] resolves to at
    // execute (fwd_base = #pre-validation targets = 1).
    const forwardConfig = {
      inputMint: USDC_MINT,
